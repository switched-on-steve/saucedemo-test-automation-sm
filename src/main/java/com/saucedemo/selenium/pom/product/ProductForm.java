...
public class ProductForm extends LoginForm {
    ...
    private boolean isInAscending(Object prevElem, Object currElem) {
        if (prevElem instanceof String) return ((String)prevElem).compareTo((String)currElem) < 0;
    ...
    public boolean checkProductSortOrder(boolean isName, boolean isAsc) {
        ...
        for (WebElement elem: products) {
            if (isName) {
                ...
                if (!firstProduct) {
                    if (isAsc) {
                        if (!isInAscending(prevProductName, currentProductName) && !(prevProductName.equals(currentProductName))) return false;
                    } else {
                        if (isInAscending(prevProductName, currentProductName) && !(prevProductName.equals(currentProductName))) return false;
                    }
                }
                ...
                if (!firstProduct) {
                    if (isAsc) {
                        if (!isInAscending(prevPrice, currentPrice) && !(Math.abs(prevPrice - currentPrice) <= eps)) return false;
                    } else {
                        if (isInAscending(prevPrice, currentPrice) && !(Math.abs(prevPrice - currentPrice) <= eps)) return false;
                    }
                }
    ...
    public void removeProductAtIndexOf(int index, String removeText) {
        ...
        if (label.equals(removeText.toLowerCase())) productButton.click();
    }
    ...
    public void addProductAtIndexOf(int index, String addText) {
        WebElement productButton = products.get(index).findElement(By.className("btn_inventory"));
        String label = productButton.getText().toLowerCase();
        if (label.equals(addText.toLowerCase())) productButton.click();
    }
    ...
    public void logout() {
        menuButton.click();
        WebElement element = driver.findElement(By.id("logout_sidebar_link"));
